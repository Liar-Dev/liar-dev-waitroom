...
public class WaitRoomServiceImpl implements WaitRoomService {
    ...
    public void saveWaitRoomComplete(WaitRoom waitRoom) {
        ...
        waitRoom.getMembers().stream().forEach(m -> waitRoomCompleteJoinMemberRepository
                .save(WaitRoomCompleteJoinMember.of(waitRoomComplete, m)));
    }
    ...
    @Override
    public Slice<WaitRoom> findAllWaitRoomsSlice(Pageable pageable) {
        return waitRoomRedisRepository.findAllBy(pageable);
    }

    @Override
    public List<WaitRoomViewsDto> findAllWaitRooms(Pageable pageable) {
        return findAllWaitRoomsSlice(pageable)
                .stream()
                .map(room -> WaitRoomViewsDto.of(room))
